...
# ─────────────────────────────────────────────────────────────
# CORS (FIXED)
# - If you are NOT using cookies/sessions, keep allow_credentials=False.
# - For production, replace "*" with your frontend domain(s).
# ─────────────────────────────────────────────────────────────
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],          # e.g. ["https://fact-check.up.railway.app"]
    allow_credentials=False,      # IMPORTANT: cannot be True when allow_origins=["*"]
    allow_methods=["*"],
    allow_headers=["*"],
)
...
# If you want Railway not to crash when keys are missing, you can comment these out,
# but it's better to set the variables correctly in Railway.
if not GEMINI_API_KEY:
    raise ValueError("❌ GEMINI_API_KEY environment variable not set")
if not SERPER_API_KEY:
    raise ValueError("❌ SERPER_API_KEY environment variable not set")
...
# Serve frontend at /app (NOT /), so it doesn't conflict with API routes.
frontend_path = os.path.join(os.path.dirname(__file__), "..", "Frontend")
if os.path.exists(frontend_path):
    app.mount("/app", StaticFiles(directory=frontend_path, html=True), name="frontend")
    print(f"✅ Frontend mounted at /app from: {frontend_path}")
else:
